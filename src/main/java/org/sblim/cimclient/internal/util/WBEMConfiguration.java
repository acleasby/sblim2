 ...
 * 2016-05-05   acleasby    Add ability to load configuration from a classpath resource
 */
...
public class WBEMConfiguration {
        ...
        if (configURL != null) {
            if (configURL.trim().length() > 0) {
                try {
                    ...
                    if (configURL.toLowerCase(Locale.getDefault()).startsWith("classpath:")) {
                        return loadClasspathURL(configURL);
                    } else {
    ...
    private static InputStream loadClasspathURL(String classpathURL) {
        String resourceName = classpathURL.substring(10).trim();
        if (resourceName.isEmpty()) {
            throw new IllegalArgumentException("Invalid classpath URL: " + classpathURL);
        }
        return WBEMConfiguration.class.getClassLoader().getResourceAsStream(resourceName);
    }
     ...
     * Ctor.
     *
     * @param pDomainProperties The domain specific properties of this configuration. Domain
     *                          specific properties are treated as an overlay on the global
     *                          properties in the <code>System</code> class.
     * @throws NullPointerException If pDomainProperties is <code>null</code>
     */
     ...
     *
     * @param pDomainProperties The new value
     */
     ...
     *
     * @param pKey   The name of property
     ...
     *
     * @param pLocalProperties The new value
     */
